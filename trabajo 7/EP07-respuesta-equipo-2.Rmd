...
Como se cumplen las condiciones podemos aplicar la prueba de suma de rangos de Wilcoxon.
...
wilcoxon
```
Debido a que el valor p es menor a el nivel de sigificancia (p < 0.05), por lo tanto, con un 95% de confianza se rechaza la hipotesis nula en favor de la hipotesis alternativa, por lo que se pude decir que hay diferencias entre los tiempos de los algoritmos A y B.
...
Como son 3 muestras se utilizara ANOVA para muestras independientes, por lo mismo se revisara si los datos cumplen con las condiciones, dichas condiciones son:

1.- "La escala con la que se mide la variable dependiente tiene las propiedades de una escala de intervalos iguale"
 Esto se puede ver, ya que, las muestras son datos de tiempo.
 
2.- "Muestras independientes"
 Esto se puede ver ya que la muestra fue elegida de forma aleatoria, por lo que se puede asumir que las observaciones si son independientes entre si.
 
3.- "Las muestras tienen una distribucion normal"
  Para probar normalidad realizaremos una prueba de Shapiro para cada muestra de manera independiente
...
normalidad_datos_C <- shapiro.test(datos_C)
normalidad_datos_C
```

Debido a que el valor p de los datos es menor a cualquier nivel de significancia usual, podemos decir que los datos A no siguen una distribución normal, por lo tanto, no podemos una prueba parametrica. Por lo que ocuparemos la prueba de Kruskal-Wallis, entonces revisaremos las condiciones:

1.- "La variable independiente debe tener a lo menos dos niveles."
  Hay 3 muestras, por lo que se cumple la condición
...
Como se cumplen las condiciones podemos aplicar la prueba de Kruskal-Wallis

```{r}
Tiempo <- c(datos_A, datos_B, datos_C)

Algoritmo <- c(rep("A", length(datos_A)),
               rep("B", length(datos_B)),
               rep("C", length(datos_C)))

Algoritmo <- factor(Algoritmo)

datos_Kruskal_Wallis <- data.frame(Tiempo, Algoritmo)
...
Debido a que el valor p es menor a el nivel de sigificancia (p < 0.05), por lo tanto, podemos decir con un 95% de confianza  que se rechaza la hipotesis nula en favor de la hipotesis alternativa, por lo que se pude decir que hay diferencias entre los tiempos de los algoritmos A, B y C.

Debido a esto se realizara una prueba post-hoc de Holm para ver cuales algoritmos presentan diferencias.
```{r}
post_hoc <- pairwise.wilcox.test(datos_Kruskal_Wallis$Tiempo,
                                 datos_Kruskal_Wallis$Algoritmo,
                                 p.adjust.method = "holm",
                                 paired = FALSE)
post_hoc
```
A partir de los resultados del procedimiento post-hoc, considerando un nivel de significancia de 0.05%, podemos concluir con un 95% de confianza que existen diferencias significativas entre los tiempos de ejecución de todos los pares de algortimos con excepción de los algoritmos A y C.
