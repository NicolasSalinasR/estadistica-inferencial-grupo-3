...
en primera instancia se realizara una tabla con los datos en el enunciado, esto para en primera instancia ordenarlos
...
```
una vez tenemos los datos ordenados, se debe revisar si sus valores esperados son mayores a 5, o no
```{r}
#obtener valores esperados

producto_sumas_dividido <- function(x) {
  resultado_multiplicacion <- x / 24
  return(resultado_multiplicacion)
}

# Aplicar la función elemento a elemento
resultados <- apply(tabla, c(1,2), producto_sumas_dividido)

print(resultados)
```
como se puede apreciar, los valores esperados de cada instancia son menores a 5, por lo cual se debe aplicar la prueba de fisher, para lo cual se consideraran las siguientes hipotesis:

HO: el tipo de chocolate preferido esta influenciado por el grupo etario de los consumidores

Ha: el tipo de chocolate preferido no esta influenciado por el grupo etario de los consumudores
...
```
en conclucion, como se puede observar el p-value es mayor que el nivel de significancia, por lo cual con un 95% de confiabilidad se falla en rechazar la hipotesis nula
